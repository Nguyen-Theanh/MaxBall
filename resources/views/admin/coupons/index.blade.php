...
<div class="px-8 py-6">
    ...
    <form action="{{ route('admin.coupons.index') }}" method="GET" class="mb-4 flex flex-col gap-3 rounded-xl border border-gray-100 bg-white p-4 shadow-sm lg:flex-row lg:items-end">
        <div class="flex w-full flex-col gap-1.5 sm:w-64">
            ...
            <select id="source" name="source" class="w-full rounded-lg border-gray-300 text-sm focus:border-blue-500 focus:ring-blue-500">
        ...
        <div class="flex w-full flex-col gap-1.5 sm:w-64">
            <label for="availability" class="text-sm font-semibold text-gray-700">Tình trạng sử dụng</label>
            <select id="availability" name="availability" class="w-full rounded-lg border-gray-300 text-sm focus:border-blue-500 focus:ring-blue-500">
                <option value="active" @selected(request('availability', 'active') === 'active')>Đang hoạt động</option>
                <option value="inactive" @selected(request('availability') === 'inactive')>Không còn hoạt động</option>
            </select>
        </div>
        <div class="flex items-center gap-2">
            ...
            @if(request()->filled('source') || request('availability', 'active') !== 'active')
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($coupons as $coupon)
                        <tr class="hover:bg-gray-50 transition-colors">
                            ...
                            <td class="py-3 px-4">
                                @if($coupon->is_currently_available)
                                    <form action="{{ route('admin.coupons.toggle-status', $coupon->id) }}" method="POST" class="inline-block">
                                        @csrf
                                        <button type="submit" class="relative inline-flex items-center h-6 rounded-full w-11 transition-colors focus:outline-none bg-green-500">
                                            <span class="inline-block w-4 h-4 transform bg-white rounded-full transition-transform translate-x-6"></span>
                                        </button>
                                    </form>
                                @endif
                                <div class="mt-1 text-xs font-semibold {{ $coupon->is_currently_available ? 'text-green-600' : 'text-gray-400' }}">
                                    {{ $coupon->availability_label }}
                                </div>
                            </td>
                            <td class="py-3 px-4 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    @if($coupon->has_orders)
                                        <span class="text-xs font-semibold text-gray-400">Đã phát sinh đơn</span>
                                    @else
